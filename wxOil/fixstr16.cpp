...
String_16::String_16(const TCHAR* psz)
{
	*(text = fixedbuf) = 0;
	length = FIX_LEN_BUFSIZE;
	if (psz != 0)
	{
		ERROR3IF(camStrlen(psz) >= length, "String_16::String_16 overflow");
		camStrcpy(text, psz);
	}
}
...
/**************************************************************************************
>	String_16::String_16(const StringBase& other)
...
/**************************************************************************************
>	String_16::String_16(UINT32 resID, UINT32 hinst = 0)
...
String_16::String_16(UINT32 resID, UINT32 hinst)
{
	*(text = fixedbuf) = 0;
	length = FIX_LEN_BUFSIZE;
	if (!Load(resID, hinst))
	{
		...
		camStrcpy(text, (camStrlen(buf) <= 16) ? buf : TEXT("!"));
	}
}
...
String_16::String_16(const wxString& wxs)
{
	*(text = fixedbuf) = 0;
	length = FIX_LEN_BUFSIZE;
	ERROR3IF(wxs.Len() >= length, "String_16::String_16 overflow");
...
/**************************************************************************************
>	virtual BOOL String_16::Alloc(INT32 nSize)
...
BOOL String_16::Alloc(INT32 nSize)
{
