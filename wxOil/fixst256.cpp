...
String_256::String_256(const TCHAR* psz)
{
	*(text = fixedbuf) = 0;
	length = FIX_LEN_BUFSIZE;
	if (psz != 0)
	{
		ERROR3IF(camStrlen(psz) >= length, "String_256::String_256 overflow");
		camStrcpy(text, psz);
	}
}
...
/**************************************************************************************
>	String_256::String_256( PCSTR psz )
...
String_256::String_256( PCSTR psz )
{
	*(text = fixedbuf) = 0;
	...
	if (psz != 0)
	{
		ERROR3IF( strlen(psz) >= length, "String_256::String_256 overflow" );
...
/**************************************************************************************
>	String_256::String_256(const StringBase& other)
...
/**************************************************************************************
>	String_256::String_256(UINT32 resID, UINT32 hinst = 0)
...
String_256::String_256(UINT32 resID, UINT32 hinst)
{
	*(text = fixedbuf) = 0;
	length = FIX_LEN_BUFSIZE;
	if (!Load(resID, hinst))
	{
		...
		camStrcpy(text, (camStrlen(buf) <= 256) ? buf : TEXT("!"));
	}
}
...
String_256::String_256(const wxString& wxs)
{
	*(text = fixedbuf) = 0;
	length = FIX_LEN_BUFSIZE;
	ERROR3IF(wxs.Len() >= length, "String_256::String_256 overflow");
...
/**************************************************************************************
>	virtual BOOL String_256::Alloc(INT32 nSize)
...
BOOL String_256::Alloc(INT32 nSize)
{
