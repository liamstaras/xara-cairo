...
String_64::String_64(const TCHAR* psz)
{
	*(text = fixedbuf) = 0;
	length = FIX_LEN_BUFSIZE;
	if (psz != 0)
	{
		ERROR3IF(camStrlen(psz) >= length, "String_64::String_64 overflow");
		camStrcpy(text, psz);
	}
}
...
/**************************************************************************************
>	String_64::String_64(const StringBase& other)
...
/**************************************************************************************
>	String_64::String_64(UINT32 resID, UINT32 hinst = 0)
...
String_64::String_64(UINT32 resID, UINT32 hinst)
{
	*(text = fixedbuf) = 0;
	length = FIX_LEN_BUFSIZE;
	if (!Load(resID, hinst))
	{
		...
		camStrcpy(text, (camStrlen(buf) <= 64) ? buf : TEXT("!"));
	}
}
...
/**************************************************************************************
>	String_64::String_64(const wxString& wxs)
...
String_64::String_64(const wxString& wxs)
{
	*(text = fixedbuf) = 0;
	length = FIX_LEN_BUFSIZE;
	ERROR3IF(wxs.Len() >= length, "String_64::String_64 overflow");
...
/**************************************************************************************
>	virtual BOOL String_64::Alloc(INT32 nSize)
...
BOOL String_64::Alloc(INT32 nSize)
{
