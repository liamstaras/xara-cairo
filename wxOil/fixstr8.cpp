...
String_8::String_8(const TCHAR* psz)
{
	*(text = fixedbuf) = 0;
	length = FIX_LEN_BUFSIZE;
	if (psz != 0)
	{
		ERROR3IF(camStrlen(psz) >= length, "String_8::String_8 overflow");
		camStrcpy(text, psz);
	}
}
...
/**************************************************************************************
>	String_8::String_8(const StringBase& other)
...
/**************************************************************************************
>	String_8::String_8(UINT32 resID, UINT32 hinst = 0)
...
String_8::String_8(UINT32 resID, UINT32 hinst)
{
	*(text = fixedbuf) = 0;
	length = FIX_LEN_BUFSIZE;
	if (!Load(resID, hinst))
	{
		...
		camStrcpy(text, (camStrlen(buf) <= 8) ? buf : TEXT("!"));
	}
}
...
/**************************************************************************************
>	String_8::String_8(const wxString& wxs)
...
String_8::String_8(const wxString& wxs)
{
	*(text = fixedbuf) = 0;
	length = FIX_LEN_BUFSIZE;
	ERROR3IF(wxs.Len() >= length, "String_8::String_8 overflow");
...
/**************************************************************************************
>	virtual BOOL String_8::Alloc(INT32 nSize)
...
BOOL String_8::Alloc(INT32 nSize)
{
